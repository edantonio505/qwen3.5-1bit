...
BONSAI_SYSTEM_PROMPT="You are Bonsai, a helpful language model created by PrismML. Follow the user's instructions and provide detailed, thoughtful responses."
...
# ── Add CUDA toolkit libs to LD_LIBRARY_PATH (needed when ldconfig is incomplete) ──
setup_cuda_ld_path() {
    _cuda_lib=""
    if command -v nvcc >/dev/null 2>&1; then
        _cuda_home="$(dirname "$(dirname "$(command -v nvcc)")")"
        for _d in "$_cuda_home/lib64" "$_cuda_home/lib"; do
            [ -d "$_d" ] && _cuda_lib="$_d" && break
        done
    elif [ -d /usr/local/cuda/lib64 ]; then
        _cuda_lib="/usr/local/cuda/lib64"
    fi
    if [ -n "$_cuda_lib" ]; then
        export LD_LIBRARY_PATH="${LD_LIBRARY_PATH:+$LD_LIBRARY_PATH:}$_cuda_lib"
    fi
}
